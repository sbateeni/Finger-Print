...
def calculate_grid_match_score(minutiae1, minutiae2, img1, img2):
    """حساب درجة تطابق المربع مع البصمة الجزئية"""
    # استخراج الخصائص
    ...
    try:
        ...
        for row in range(grid_size):
            for col in range(grid_size):
                ...
                grid = processed[y1:y2, x1:x2].copy()
                
                # حفظ المربع
                grid_filename = f"{timestamp}_grid_{row+1}_{col+1}.png"
                grid_path = os.path.join(PROCESSED_FOLDER, grid_filename)
                cv2.imwrite(grid_path, grid)
                
                grid_images.append(url_for('static', filename=f'images/processed/{grid_filename}'))
...
def compare_grids():
    try:
        ...
        # Get the grid images from the processed folder
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        grid1_path = os.path.join(PROCESSED_FOLDER, f'{timestamp}_grid_{grid1_index+1}.png')
        grid2_path = os.path.join(PROCESSED_FOLDER, f'{timestamp}_grid_{grid2_index+1}.png')
        
        if not (os.path.exists(grid1_path) and os.path.exists(grid2_path)):
            return jsonify({'error': 'لم يتم العثور على المربعات المحددة'}), 404
        
        # Process the grid images
        grid1 = cv2.imread(grid1_path, cv2.IMREAD_GRAYSCALE)
        grid2 = cv2.imread(grid2_path, cv2.IMREAD_GRAYSCALE)
        
        # Extract features and minutiae
        features1 = extract_features(grid1)
        features2 = extract_features(grid2)
        minutiae1 = extract_minutiae(grid1)
        minutiae2 = extract_minutiae(grid2)
        ...
        quality1 = analyze_match_quality({'quality_score': len(minutiae1) / 100})['quality_score']
        quality2 = analyze_match_quality({'quality_score': len(minutiae2) / 100})['quality_score']
        
        # Calculate average score and matching squares
        avg_score = score_details['total_score']
        matching_squares = int(score_details['total_score'] * grid_size * grid_size)
        
        return jsonify({
            'match_score': float(score_details['total_score']),
            'avg_score': float(avg_score),
            'matching_squares': matching_squares,
